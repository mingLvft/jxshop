            ...
            <?php if(is_array($info)): $keys = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($keys % 2 );++$keys;?><tr align="center" class="0">
               <?php if(is_array($attr)): $i = 0; $__LIST__ = $attr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><td align="left" class="first-cell" >
                    <select name="attr[<?php echo ($vo["0"]["attr_id"]); ?>][]">
                        <?php if(is_array($vo)): $i = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if(in_array(($v["id"]), is_array($v2["goods_attr_ids"])?$v2["goods_attr_ids"]:explode(',',$v2["goods_attr_ids"]))): ?>selected="selected"<?php endif; ?>><?php echo ($v["attr_values"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
